...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "spiky", "tests")); system.time(test_check("spiky", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran spiky_$(VERSION).tar.gz
	rm -rf spiky.Rcheck/
